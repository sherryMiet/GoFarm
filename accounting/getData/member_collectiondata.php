...
$id =$decode['collection']['id'];//把email從user抓出來
...
$sql = "SELECT m_email,m_name,m_followers,m_email,col_title,col_billboard,col_article
        FROM member AS M , collection AS C
        WHERE C.col_id = $id AND C.col_m_id = M.m_id";
...
while($row = mysqli_fetch_assoc($result)){
    $array = array("m_email" => $row['m_email'],//將資料城陣列
        ...
        "col_billboard" => $row['col_billboard'],
        "col_article" => $row['col_article'],
    );
}
